...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Kiểm tra display_order đã tồn tại chưa (ngoại trừ chính banner này)
    if (empty($errors) && $display_order > 0) {
        $check_order_sql = "SELECT banner_id FROM banners WHERE display_order = ? AND banner_id != ?";
        $check_stmt = $conn->prepare($check_order_sql);
        $check_stmt->bind_param("ii", $display_order, $banner_id);
        $check_stmt->execute();
...
// Lấy danh sách display_order đã sử dụng để hiển thị gợi ý (ngoại trừ banner hiện tại)
$used_orders_sql = "SELECT display_order FROM banners WHERE display_order > 0 AND banner_id != ? ORDER BY display_order ASC";
$used_orders_stmt = $conn->prepare($used_orders_sql);
$used_orders_stmt->bind_param("i", $banner_id);
$used_orders_stmt->execute();
$used_orders_result = $used_orders_stmt->get_result();
$used_orders = [];
                                ...
                                <form method="POST" id="bannerForm">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea name="banner_description" 
                                                  class="form-control" 
                                                  rows="4"
                                                  placeholder="Nhập mô tả chi tiết về banner, mục đích sử dụng..."><?php echo htmlspecialchars($banner['banner_description']); ?></textarea>
                                        <div class="help-text">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="number" 
                                               ...
                                               id="displayOrder">
                                        <div class="help-text">
                                            Thứ tự hiển thị của banner (số nhỏ hơn sẽ hiển thị trước)
                                        </div>
                                        ...
                                        <div class="order-suggestion">
                                            ...
                                            <div>
                                                ...
                                                <span class="order-current"><?php echo $banner['display_order']; ?></span>
                                            </div>
                                            <?php if (!empty($used_orders)): ?>
                                            <div style="margin-top: 10px;">
                                                <strong>Đã sử dụng bởi banner khác:</strong>
                                            ...
                                            <div class="order-available">
                                                ...
                                                Gợi ý: Có thể giữ nguyên <?php echo $banner['display_order']; ?> hoặc chọn số khác chưa được sử dụng
                                            </div>
                                        </div>
                                    </div>
    ...
    <script>
        ...
        document.getElementById('bannerUrl').addEventListener('input', function() {
            ...
            if (url && isValidUrl(url)) {
                ...
                previewText.textContent = url ? 'URL không hợp lệ' : 'Thay đổi URL để xem trước hình ảnh mới';
                previewText.style.display = 'block';
                previewImage.style.display = 'none';
            }
        });
        ...
        const currentOrder = <?php echo $banner['display_order']; ?>;
        
        document.getElementById('displayOrder').addEventListener('input', function() {
            const value = parseInt(this.value);
            if (value > 0 && value !== currentOrder && usedOrders.includes(value)) {
                ...
                warning.innerHTML = '<i class="fas fa-exclamation-triangle me-1"></i>Thứ tự ' + value + ' đã được sử dụng bởi banner khác!';
            } else {
        ...
        document.getElementById('bannerForm').addEventListener('submit', function(e) {
            ...
            if (displayOrder > 0 && displayOrder !== currentOrder && usedOrders.includes(displayOrder)) {
                e.preventDefault();
                alert('Thứ tự hiển thị ' + displayOrder + ' đã được sử dụng bởi banner khác! Vui lòng chọn số khác.');
                return;
            }
        });
    </script>
